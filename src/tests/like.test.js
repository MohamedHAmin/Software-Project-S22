...
test("Like a tweet", async () => {
    ...
    const res = await request(app).put('/tweet/'+newtweet._id+'/like')
...
const res2 = await request(app).put('/tweet/'+newtweet._id+'/like')
    .set('Authorization','Bearer '+usertoken.token)
    .expect(201)
});
...
test("Like a non existent tweet", async () => {
  const res = await request(app).put('/tweet/12345678/like')
  .set('Authorization','Bearer '+usertoken.token)
  .expect(400)
