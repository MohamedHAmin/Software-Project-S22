...
public class createTweetMoreThan280CharacterAndroid {
    ...
    public void CreateTweetMoreThan280() throws InterruptedException{
        ...
        writeTweet.sendKeys(tweetBodyMoreThan280);
        // Find tweet button then click on it.
        WebElement tweetButton = (new WebDriverWait(driver,60)).until(ExpectedConditions.presenceOfElementLocated(new By.ById(tweetButtonId)));
        // Check if the tweet button is enabled or not.
        if(tweetButton.isEnabled()) {
            System.out.println(testFailed); }
        else { System.out.println(testPass); }
        // Check if the tweet character warning count matches the expected one.
        // Find the tweet character warning count.
        WebElement tweetCharacterCount = (new WebDriverWait(driver,60)).until(ExpectedConditions.presenceOfElementLocated(new By.ById(countCharacterId)));
    ...
    public void tearDown() throws InterruptedException {
        ...
        WebElement logoutButton = (new WebDriverWait(driver,60)).until(ExpectedConditions.presenceOfElementLocated(new By.ByXPath(loginButtonXpath)));
        logoutButton.click();
